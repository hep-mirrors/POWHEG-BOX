...
plot infil1 index  iindex using ($1+$2)/2:($3*scale):1:2:(scale*($3-$4)):(scale*($3+$4)) title mytitle,\
...
using ($1+$2)/2:($3*scale/$7):1:2:($3*scale/$7)*(1-(($4/$3)**2+($8/$7)**2)**0.5):($3*scale/$7)*(1+(($4/$3)**2+($8/$7)**2)**0.5) title mytitle, 1 with lines linestyle -1
