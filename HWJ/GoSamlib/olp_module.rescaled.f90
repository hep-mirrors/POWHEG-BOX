...
contains
   ...
   & bind(C,name="olp_evalsubprocess_")
      use, intrinsic :: iso_c_binding
     use p6_ubbar_hepneg_model, only:  p6_ubbar_hepneg_mH  => mH, p6_ubbar_hepneg_mW  => mW, &
          & p6_ubbar_hepneg_mZ  => mZ, p6_ubbar_hepneg_wW  => wW, p6_ubbar_hepneg_GF  => GF, &
          & p6_ubbar_hepneg_mT  => mT
     use p12_cbbar_hepneg_model, only: p12_cbbar_hepneg_mH => mH, p12_cbbar_hepneg_mW  => mW, &
          & p12_cbbar_hepneg_mZ  => mZ, p12_cbbar_hepneg_wW  => wW, p12_cbbar_hepneg_GF  => GF, &
          & p12_cbbar_hepneg_mT  => mT
     use p5_usbar_hepneg_model, only:  p5_usbar_hepneg_mH  => mH, p5_usbar_hepneg_mW  => mW, &
          & p5_usbar_hepneg_mZ  => mZ, p5_usbar_hepneg_wW  => wW, p5_usbar_hepneg_GF  => GF, &
          & p5_usbar_hepneg_mT  => mT
     use p1_dbarc_hepneg_model, only:  p1_dbarc_hepneg_mH  => mH, p1_dbarc_hepneg_mW  => mW, &
          & p1_dbarc_hepneg_mZ  => mZ, p1_dbarc_hepneg_wW  => wW, p1_dbarc_hepneg_GF  => GF, &
          & p1_dbarc_hepneg_mT  => mT
     use p11_csbar_hepneg_model, only: p11_csbar_hepneg_mH => mH, p11_csbar_hepneg_mW  => mW, &
          & p11_csbar_hepneg_mZ  => mZ, p11_csbar_hepneg_wW  => wW, p11_csbar_hepneg_GF  => GF, &
          & p11_csbar_hepneg_mT  => mT
     use p0_dbaru_hepneg_model, only:  p0_dbaru_hepneg_mH  => mH, p0_dbaru_hepneg_mW  => mW, &
          & p0_dbaru_hepneg_mZ  => mZ, p0_dbaru_hepneg_wW  => wW, p0_dbaru_hepneg_GF  => GF, &
          & p0_dbaru_hepneg_mT  => mT
      use p0_dbaru_hepneg_kinematics, only: dotproduct
      implicit none
      ...
      real(kind=c_double) :: alpha_s, hmass, ecms, rescale, mux
      real(kind=c_double), dimension(50) :: momentax
      real(kind=c_double) :: mW_tmp, mZ_tmp, wW_tmp, mT_tmp, GF_tmp
      real(kind=c_double), parameter :: one_over_2pi = 0.15915494309189533577d0
      real(kind=c_double), dimension(4) :: res_tmp

      hmass = sqrt(momenta(11)**2 - momenta(12)**2 - momenta(13)**2 - momenta(14)**2)
      ecms  = sqrt(2d0 * dotproduct(momenta(1:4), momenta(6:9)))
      ...
      select case(label)
      case(0)
              call eval34(momenta(1:30), mu, parameters, res_tmp)
      case(1)
              call eval6(momenta(1:30), mu, parameters, res_tmp)
      case(2)
              call eval9(momenta(1:30), mu, parameters, res_tmp)
      case(3)
              call eval20(momenta(1:30), mu, parameters, res_tmp)
      case(4)
              call eval22(momenta(1:30), mu, parameters, res_tmp)
      case(5)
              call eval28(momenta(1:30), mu, parameters, res_tmp)
      case(6)
              call eval35(momenta(1:30), mu, parameters, res_tmp)
      case(7)
              call eval12(momenta(1:30), mu, parameters, res_tmp)
      case(8)
              call eval15(momenta(1:30), mu, parameters, res_tmp)
      case(9)
              call eval21(momenta(1:30), mu, parameters, res_tmp)
      case(10)
              call eval23(momenta(1:30), mu, parameters, res_tmp)
      case(11)
              call eval31(momenta(1:30), mu, parameters, res_tmp)
      case(12)
              call eval32(momenta(1:30), mu, parameters, res_tmp)
      case(13)
              call eval5(momenta(1:30), mu, parameters, res_tmp)
      case(14)
              call eval8(momenta(1:30), mu, parameters, res_tmp)
      case(15)
              call eval16(momenta(1:30), mu, parameters, res_tmp)
      case(16)
              call eval18(momenta(1:30), mu, parameters, res_tmp)
      case(17)
              call eval27(momenta(1:30), mu, parameters, res_tmp)
      case(18)
              call eval1(momenta(1:30), mu, parameters, res_tmp)
      case(19)
              call eval3(momenta(1:30), mu, parameters, res_tmp)
      case(20)
              call eval10(momenta(1:30), mu, parameters, res_tmp)
      case(21)
              call eval13(momenta(1:30), mu, parameters, res_tmp)
      case(22)
              call eval25(momenta(1:30), mu, parameters, res_tmp)
      case(23)
              call eval29(momenta(1:30), mu, parameters, res_tmp)
      case(24)
              call eval33(momenta(1:30), mu, parameters, res_tmp)
      case(25)
              call eval11(momenta(1:30), mu, parameters, res_tmp)
      case(26)
              call eval14(momenta(1:30), mu, parameters, res_tmp)
      case(27)
              call eval17(momenta(1:30), mu, parameters, res_tmp)
      case(28)
              call eval19(momenta(1:30), mu, parameters, res_tmp)
      case(29)
              call eval30(momenta(1:30), mu, parameters, res_tmp)
      case(30)
              call eval0(momenta(1:30), mu, parameters, res_tmp)
      case(31)
              call eval2(momenta(1:30), mu, parameters, res_tmp)
      case(32)
              call eval4(momenta(1:30), mu, parameters, res_tmp)
      case(33)
              call eval7(momenta(1:30), mu, parameters, res_tmp)
      case(34)
              call eval24(momenta(1:30), mu, parameters, res_tmp)
      case(35)
              call eval26(momenta(1:30), mu, parameters, res_tmp)
      case default
         res_tmp(:) = 0.0d0
      end select
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      mW_tmp = p0_dbaru_hepneg_mW
      mZ_tmp = p0_dbaru_hepneg_mZ
      wW_tmp = p0_dbaru_hepneg_wW
      GF_tmp = p0_dbaru_hepneg_GF
      mT_tmp = p0_dbaru_hepneg_mT

      rescale = 1d0/ecms
      ! write(*,*) "rescale", rescale
      ! write(*,*) "ecms", ecms
      mux = mu*rescale
      momentax(:) = momenta(:)*rescale

      p6_ubbar_hepneg_mH  = hmass*rescale
      p12_cbbar_hepneg_mH = hmass*rescale
      p5_usbar_hepneg_mH  = hmass*rescale
      p1_dbarc_hepneg_mH  = hmass*rescale
      p11_csbar_hepneg_mH = hmass*rescale
      p0_dbaru_hepneg_mH  = hmass*rescale

      p6_ubbar_hepneg_mW  = mW_tmp*rescale
      p12_cbbar_hepneg_mW = mW_tmp*rescale
      p5_usbar_hepneg_mW  = mW_tmp*rescale
      p1_dbarc_hepneg_mW  = mW_tmp*rescale
      p11_csbar_hepneg_mW = mW_tmp*rescale
      p0_dbaru_hepneg_mW  = mW_tmp*rescale

      p6_ubbar_hepneg_mZ  = mZ_tmp*rescale
      p12_cbbar_hepneg_mZ = mZ_tmp*rescale
      p5_usbar_hepneg_mZ  = mZ_tmp*rescale
      p1_dbarc_hepneg_mZ  = mZ_tmp*rescale
      p11_csbar_hepneg_mZ = mZ_tmp*rescale
      p0_dbaru_hepneg_mZ  = mZ_tmp*rescale

      p6_ubbar_hepneg_wW  = wW_tmp*rescale
      p12_cbbar_hepneg_wW = wW_tmp*rescale
      p5_usbar_hepneg_wW  = wW_tmp*rescale
      p1_dbarc_hepneg_wW  = wW_tmp*rescale
      p11_csbar_hepneg_wW = wW_tmp*rescale
      p0_dbaru_hepneg_wW  = wW_tmp*rescale

      p6_ubbar_hepneg_GF  = GF_tmp/rescale/rescale
      p12_cbbar_hepneg_GF = GF_tmp/rescale/rescale
      p5_usbar_hepneg_GF  = GF_tmp/rescale/rescale
      p1_dbarc_hepneg_GF  = GF_tmp/rescale/rescale
      p11_csbar_hepneg_GF = GF_tmp/rescale/rescale
      p0_dbaru_hepneg_GF  = GF_tmp/rescale/rescale

      p6_ubbar_hepneg_mT  = mT_tmp*rescale
      p12_cbbar_hepneg_mT = mT_tmp*rescale
      p5_usbar_hepneg_mT  = mT_tmp*rescale
      p1_dbarc_hepneg_mT  = mT_tmp*rescale
      p11_csbar_hepneg_mT = mT_tmp*rescale
      p0_dbaru_hepneg_mT  = mT_tmp*rescale

      ! write(*,*)"--> mH in OLP: ",momenta(15)
      ! write(*,*)"--> new mH in OLP: ",hmass
      ! write(*,*)"--> GF in OLP: ", GF
      ! write(*,*)"--> mW in OLP: ", mW
      ! write(*,*)"--> mZ in OLP: ", mZ
      ! write(*,*)"--> wW in OLP: ", wW
      ! write(*,*)"--> wZ in OLP: ", wZ
      ! write(*,*)"--> mT in OLP: ", mT
      ! write(*,*)"--> CVDU in OLP: ", CVDU

      select case(label)
      case(0)
              call eval34(momentax(1:30), mux, parameters, res)
      case(1)
              call eval6(momentax(1:30), mux, parameters, res)
      case(2)
              call eval9(momentax(1:30), mux, parameters, res)
      case(3)
              call eval20(momentax(1:30), mux, parameters, res)
      case(4)
              call eval22(momentax(1:30), mux, parameters, res)
      case(5)
              call eval28(momentax(1:30), mux, parameters, res)
      case(6)
              call eval35(momentax(1:30), mux, parameters, res)
      case(7)
              call eval12(momentax(1:30), mux, parameters, res)
      case(8)
              call eval15(momentax(1:30), mux, parameters, res)
      case(9)
              call eval21(momentax(1:30), mux, parameters, res)
      case(10)
              call eval23(momentax(1:30), mux, parameters, res)
      case(11)
              call eval31(momentax(1:30), mux, parameters, res)
      case(12)
              call eval32(momentax(1:30), mux, parameters, res)
      case(13)
              call eval5(momentax(1:30), mux, parameters, res)
      case(14)
              call eval8(momentax(1:30), mux, parameters, res)
      case(15)
              call eval16(momentax(1:30), mux, parameters, res)
      case(16)
              call eval18(momentax(1:30), mux, parameters, res)
      case(17)
              call eval27(momentax(1:30), mux, parameters, res)
      case(18)
              call eval1(momentax(1:30), mux, parameters, res)
      case(19)
              call eval3(momentax(1:30), mux, parameters, res)
      case(20)
              call eval10(momentax(1:30), mux, parameters, res)
      case(21)
              call eval13(momentax(1:30), mux, parameters, res)
      case(22)
              call eval25(momentax(1:30), mux, parameters, res)
      case(23)
              call eval29(momentax(1:30), mux, parameters, res)
      case(24)
              call eval33(momentax(1:30), mux, parameters, res)
      case(25)
              call eval11(momentax(1:30), mux, parameters, res)
      case(26)
              call eval14(momentax(1:30), mux, parameters, res)
      case(27)
              call eval17(momentax(1:30), mux, parameters, res)
      case(28)
              call eval19(momentax(1:30), mux, parameters, res)
      case(29)
              call eval30(momentax(1:30), mux, parameters, res)
      case(30)
              call eval0(momentax(1:30), mux, parameters, res)
      case(31)
              call eval2(momentax(1:30), mux, parameters, res)
      case(32)
              call eval4(momentax(1:30), mux, parameters, res)
      case(33)
              call eval7(momentax(1:30), mux, parameters, res)
      case(34)
              call eval24(momentax(1:30), mux, parameters, res)
      case(35)
              call eval26(momentax(1:30), mux, parameters, res)
      case default
         res(:) = 0.0d0
      end select
      ...
      p6_ubbar_hepneg_mW  = mW_tmp
      p12_cbbar_hepneg_mW = mW_tmp
      p5_usbar_hepneg_mW  = mW_tmp
      p1_dbarc_hepneg_mW  = mW_tmp
      p11_csbar_hepneg_mW = mW_tmp
      p0_dbaru_hepneg_mW  = mW_tmp

      p6_ubbar_hepneg_mZ  = mZ_tmp
      p12_cbbar_hepneg_mZ = mZ_tmp
      p5_usbar_hepneg_mZ  = mZ_tmp
      p1_dbarc_hepneg_mZ  = mZ_tmp
      p11_csbar_hepneg_mZ = mZ_tmp
      p0_dbaru_hepneg_mZ  = mZ_tmp

      p6_ubbar_hepneg_wW  = wW_tmp
      p12_cbbar_hepneg_wW = wW_tmp
      p5_usbar_hepneg_wW  = wW_tmp
      p1_dbarc_hepneg_wW  = wW_tmp
      p11_csbar_hepneg_wW = wW_tmp
      p0_dbaru_hepneg_wW  = wW_tmp

      p6_ubbar_hepneg_GF  = GF_tmp
      p12_cbbar_hepneg_GF = GF_tmp
      p5_usbar_hepneg_GF  = GF_tmp
      p1_dbarc_hepneg_GF  = GF_tmp
      p11_csbar_hepneg_GF = GF_tmp
      p0_dbaru_hepneg_GF  = GF_tmp

      p6_ubbar_hepneg_mT  = mT_tmp
      p12_cbbar_hepneg_mT = mT_tmp
      p5_usbar_hepneg_mT  = mT_tmp
      p1_dbarc_hepneg_mT  = mT_tmp
