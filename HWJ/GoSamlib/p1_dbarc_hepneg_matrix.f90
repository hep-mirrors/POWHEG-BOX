module     p1_dbarc_hepneg_matrix
   ...
   use p1_dbarc_hepneg_diagramsh1l0, only: amplitude1l0 => amplitude
   use p1_dbarc_hepneg_amplitudeh1, only: samplitudeh1l1 => samplitude, &
      ...
      if(SP_rescue) then
         ...
         if(abs((amp(3)-irp(2))/amp(1)) .gt. (SP_chk_threshold2)) then
            if(SP_verbosity .ge. 2) then
               ...
               write(*,*)
               write(42,'(A7)')"<event>"
               write(42,'(A29,A30)') "Single pole check failed for:", "p1_dbarc_hepneg"
