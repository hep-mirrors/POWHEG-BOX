module     p0_dbaru_hepneg_d70h1l1d
   ...
   ! dbaru_hepneg/helicity1/d70h1l1d.f90
...
!---#[ function derivative:
   function derivative(mu2, i1, i2, i3) result(numerator)
      ...
      t3 = spvak6k2(iv1)
      if(deg.eq.1) then
         numerator = (k2(iv1)*abb70n27+k6(iv1)*abb70n23+spvak6k4(iv1)*abb70n24+s&
         &pvak6l3(iv1)*abb70n22+spval3k2(iv1)*abb70n25+t1*abb70n28+t2*abb70n21+t&
         &3*abb70n26)
         return
      end if
      t4 = spvak6k2(iv2)
      if(deg.eq.2) then
         numerator = ((2.0_ki)*d(iv1,iv2)*abb70n19+spvak1k2(iv2)*t3*abb70n18+spv&
         &ak1k4(iv1)*t4*abb70n17+spvak1k4(iv2)*t3*abb70n17+spvak5k2(iv2)*t3*abb7&
         &0n20+t1*t4*abb70n18+t2*t4*abb70n20)
         return
      end if
      if(deg.eq.3) then
         numerator = (0.0_ki)
