...
!---#[ function finite_renormalisation:
   function     finite_renormalisation(scale2) result(amp)
      ...
      use p1_dbarc_hepneg_diagramsh1l0, only: amplitudel0 => amplitude
      use p1_dbarc_hepneg_diagramsh1l0fr5, only: amplitudel0fr5 => amplitude
      implicit none
   ...
   function     samplitude(scale2,ok,opt_amp0,opt_perm)
     ...
     & only: init_lo, rat2
      use p1_dbarc_hepneg_abbrevh1, only: init_abbrev
      use p1_dbarc_hepneg_diagramsh1l0, only: amplitudel0 => amplitude
      use p1_dbarc_hepneg_groups
      implicit none
      real(ki), intent(in) :: scale2
...
subroutine     evaluate_group0(scale2,samplitude,ok)
   ...
   use p1_dbarc_hepneg_golem95h1, only: reconstruct_golem95 => reconstruct_group
   use p1_dbarc_hepneg_groups, only: contract_golem95, &
      & global_coeffs => coeffs_group0, &
      & reduce_numetens => reduce_numetens_group0
   use p1_dbarc_hepneg_samuraih1, only: samurai_reduce => reduce_group0
...
subroutine     evaluate_group1(scale2,samplitude,ok)
   ...
   use p1_dbarc_hepneg_samuraih1, only: samurai_reduce => reduce_group1
   use options, only: samurai_out => iout
   implicit none
   real(ki), intent(in) :: scale2
   logical, intent(out) :: ok
...
subroutine     evaluate_group2(scale2,samplitude,ok)
   ...
   use p1_dbarc_hepneg_samuraih1, only: samurai_reduce => reduce_group2
   use options, only: samurai_out => iout
   implicit none
   real(ki), intent(in) :: scale2
   logical, intent(out) :: ok
...
subroutine     evaluate_group3(scale2,samplitude,ok)
   ...
   use p1_dbarc_hepneg_samuraih1, only: samurai_reduce => reduce_group3
   use options, only: samurai_out => iout
   implicit none
   real(ki), intent(in) :: scale2
   logical, intent(out) :: ok
...
!---#] subroutine evaluate_group3:
end module p1_dbarc_hepneg_amplitudeh1
