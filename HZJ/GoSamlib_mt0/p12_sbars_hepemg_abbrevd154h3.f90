module     p12_sbars_hepemg_abbrevd154h3
   use p12_sbars_hepemg_config, only: ki
   use p12_sbars_hepemg_globalsh3
   implicit none
   private
   complex(ki), dimension(21), public :: abb154
   complex(ki), public :: R2d154
   public :: init_abbrev
   complex(ki), parameter :: i_ = (0.0_ki, 1.0_ki)
contains
   subroutine     init_abbrev()
      use p12_sbars_hepemg_config, only: deltaOS, &
     &    logfile, debug_nlo_diagrams
      use p12_sbars_hepemg_kinematics
      use p12_sbars_hepemg_model
      use p12_sbars_hepemg_color, only: TR
      use p12_sbars_hepemg_globalsl1, only: epspow
      implicit none
      abb154(1)=1.0_ki/(-es61-es12+es345)
      abb154(2)=1.0_ki/(-mZ**2+es345+i_*mZ*wZ)
      abb154(3)=1.0_ki/(-mZ**2+es45+i_*mZ*wZ)
      abb154(4)=NC**(-1)
      abb154(5)=spbk6e6*spbk5k1
      abb154(6)=abb154(5)*spak4k6
      abb154(7)=NC-abb154(4)
      abb154(7)=abb154(7)*gSr*gHZZ*c1*i_*TR*ger*abb154(3)*abb154(2)*abb154(1)
      abb154(8)=spak2e6*abb154(7)
      abb154(9)=abb154(6)*abb154(8)
      abb154(10)=spbk2k1*spak2k4
      abb154(5)=-spak1k6*abb154(10)*abb154(5)
      abb154(6)=es61*abb154(6)
      abb154(5)=abb154(6)+abb154(5)
      abb154(5)=2.0_ki*abb154(8)*abb154(5)
      abb154(6)=-es345+es12+es61
      abb154(11)=spak2k4*spbk5k1
      abb154(6)=abb154(11)*abb154(6)
      abb154(12)=spak1k4*spbk5k1
      abb154(13)=spak2k6*spbk6k1
      abb154(14)=-abb154(12)*abb154(13)
      abb154(6)=abb154(14)+abb154(6)
      abb154(6)=-abb154(7)*abb154(6)
      abb154(14)=spbk6k1*spak4k6
      abb154(10)=abb154(14)+abb154(10)
      abb154(14)=-spbk6k5*abb154(10)*abb154(7)
      abb154(15)=-spak2k6*abb154(14)
      abb154(6)=abb154(15)+abb154(6)
      abb154(6)=4.0_ki*abb154(6)
      abb154(10)=abb154(10)*abb154(8)
      abb154(15)=abb154(10)*spbe6k5
      abb154(16)=abb154(8)*spbe6k1
      abb154(17)=abb154(16)*abb154(12)
      abb154(15)=abb154(15)+abb154(17)
      abb154(15)=4.0_ki*abb154(15)
      abb154(17)=4.0_ki*abb154(9)
      abb154(18)=-2.0_ki*abb154(9)
      abb154(19)=abb154(7)*spae6k6
      abb154(20)=-spbk6k1*abb154(19)
      abb154(21)=abb154(8)*spbk2k1
      abb154(20)=abb154(20)-abb154(21)
      abb154(12)=abb154(20)*abb154(12)
      abb154(14)=spae6k6*abb154(14)
      abb154(10)=spbk5k2*abb154(10)
      abb154(11)=abb154(11)*abb154(19)
      abb154(19)=-abb154(8)*spak4k6*spbk5k1
      abb154(11)=abb154(19)+abb154(11)
      abb154(11)=spbk6k2*abb154(11)
      abb154(10)=abb154(11)+abb154(10)+abb154(12)+abb154(14)
      abb154(10)=2.0_ki*abb154(10)
      abb154(11)=2.0_ki*spbk6e6
      abb154(12)=abb154(11)*spak2k6
      abb154(14)=abb154(12)*abb154(21)
      abb154(7)=-8.0_ki*abb154(7)*abb154(13)
      abb154(13)=8.0_ki*abb154(16)
      abb154(16)=-4.0_ki*abb154(20)
      abb154(11)=abb154(8)*abb154(11)
      abb154(8)=spbk5k2*abb154(12)*abb154(8)
      R2d154=abb154(9)
      rat2 = rat2 + R2d154
      if (debug_nlo_diagrams) then
          write (logfile,*) "<result name='r2' index='154' value='", &
          & R2d154, "'/>"
      end if
   end subroutine
end module p12_sbars_hepemg_abbrevd154h3
