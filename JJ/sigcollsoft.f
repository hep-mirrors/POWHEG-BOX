...
c$$$      character * 15 splitting(maxalr)
c$$$      common/csplit/splitting
c$$$  splitting(alr)=' NOT DEFINED'
...
c$$$         splitting(alr)=' g->gg FSR'
cccccccccccccccccccccccccccccccccccccccccccccc
c$$$         ap=0 
c$$$         do mu=0,3
c$$$            do nu=0,3
c$$$               ap=ap+(-gtens(mu,nu)*(csi*x/(1-x)+(1-x)/xocsi)
c$$$     #+2*x*(1-x)*csi*
c$$$     #(kperp(mu)*kperp(nu))/kperp2)*br_bmunu(mu,nu,em,iub)
c$$$            enddo
c$$$         enddo
cccccccccccccccccccc
c     !: !!!!!!!!!!!!SPIN-AVERAGED AP!!!!!!!!!!!!!
         ap=br_born(iub)*
     $        (csi*x/(1-x)+(1-x)/xocsi + x*(1-x)*csi) 
cccccccccccccccccccc
         ap=ap*2*ca
c     suppress soft term
         ap=ap*(1-x)
      elseif((emflav+raflav.eq.0).and.(emflav.ne.0)) then
cccccccccccccccccccccccccccccccccccccccccccccc
c$$$         splitting(alr)=' g->qqbar FSR'
cccccccccccccccccccccccccccccccccccccccccccccc         
c$$$         ap=0d0
c$$$          do mu=0,3
c$$$            do nu=0,3
c$$$               ap=ap+(-gtens(mu,nu)
c$$$     #-4*x*(1-x)*(kperp(mu)*kperp(nu))/kperp2)*br_bmunu(mu,nu,em,iub)
c$$$            enddo
c$$$         enddo
cccccccccccccccccccc
c     !: !!!!!!!!!!!!SPIN-AVERAGED AP!!!!!!!!!!!!!
...
cccccccccccccccccccccccccccccccccccccc
      ...
      if(emflav.eq.0.and.raflav.eq.0) then
cccccccccccccccccccccccccccccccccccccccccccccc
c$$$         splitting(alr)=' g->(g)g ISR'
cccccccccccccccccccccccccccccccccccccccccccccc        
c$$$         ap=0
c$$$         do mu=0,3
c$$$            do nu=0,3
c$$$               ap=ap+(-gtens(mu,nu)*(x+x*(1-x)**2)
c$$$     #+2*(1-x)**2/x*
c$$$     #(kperp(mu)*kperp(nu)))*br_bmunu(mu,nu,i,iub)
c$$$            enddo
c$$$         enddo
cccccccccccccccccccc
c     !: !!!!!!!!!!!!SPIN-AVERAGED AP!!!!!!!!!!!!!
         ap=(x+x*(1-x)**2+(1-x)**2 /x)
     $*br_born(iub)               
cccccccccccccccccccc
         ap=ap*2*ca
      elseif(raflav-emflav.eq.0) then
cccccccccccccccccccccccccccccccccccccccccccccc
c$$$         splitting(alr)=' q->(g)q ISR'
cccccccccccccccccccccccccccccccccccccccccccccc 
c$$$         ap=0d0
c$$$         do mu=0,3
c$$$            do nu=0,3
c$$$               ap=ap+(-gtens(mu,nu)*x
c$$$     #+4*(1-x)/x*(kperp(mu)*kperp(nu)))*br_bmunu(mu,nu,i,iub)
c$$$            enddo
c$$$         enddo
cccccccccccccccccccc
c     !: !!!!!!!!!!!!SPIN-AVERAGED AP!!!!!!!!!!!!!
