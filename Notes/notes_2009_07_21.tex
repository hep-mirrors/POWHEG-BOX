...
\newcommand\POWHEGBOX{\mbox{POWHEG-BOX}}
...
\section{User supplied routines}
 ...
 \mbox{POWHEG-BOX} is a general program for the implementation of a user
 supplied NLO calculation in the  \mbox{POWHEG} framework.
...
\item
 ...
 header file in the \tmtexttt{include} directory. It must be set to the total
...
\item
 ...
 Notice: all variables with the prefix \tmtexttt{flst\_} are
...
\item from leg 3 onward: final state particles, in the order:
 Colorless particles first, massive coloured particles,
...
\end{itemize}
 ...
 \to (Z\to e^+e^-)+ 2 j$}, the string \tmtexttt{[1,0,11,-11,1,0]} labels the process $d g \to e^- e^+ d
 g$.  Notice that \emph{only one ordering of final state flavour can appear for a
 given final state}. Thus, in our example, either \tmtexttt{[1,0,11,-11,1,0]}
or \tmtexttt{[1,0,11,-11,0,1]} may appear, but not both.
\item The variable
\tmtexttt{flst\_startcol}, to be initialized by the user in
...
\item The user should fix in this routine the variable \tmtexttt{st\_nlight} to the
      ...
      Notice: all variables with the prefix \tmtexttt{st\_} are made
...
\tmtexttt{set\_ren\_fac\_scales(mur,muf)},
that returns the values of the two scales.
Here one can adopt the choice he/she likes for the two scales, even on a
event-by-event basis, since a call to \tmtexttt{set\_ren\_fac\_scales} is performed 
for each event.\\
\end{itemize}
We stress that
the \mbox{POWHEG} method requires these scales to be
...
\tmtexttt{set\_ren\_fac\_scales} is called.
...
Notice: all variables with the prefix \tmtexttt{kn\_} are made
...
\item the Born squared amplitude \tmtexttt{born}, summed on final and averaged on initial
  ...
  This corresponds to $\mathcal{B}$ in the notation of \cite{Frixione:2007vw}.
\item the Born color-correlated squared amplitudes
  ...
  exposed $\mathcal{B}_{jk}$, defined in eq.(2.97) of \cite{Frixione:2007vw}. 
...
for the Born processes. \mbox{POWHEG-BOX} takes care of filling the
...
\item
 ...
 are defined in eq.(2.92) and eq.(2.99) of \cite{Frixione:2007vw}.
...
\end{equation}  
defined as in eq.(2.93) of \cite{Frixione:2007vw}, that is factorized out.
Since \mbox{POWHEG-BOX} assumes that the pole subtraction is performed within
...
blocks of ref.~\cite{Altarelli:1989wu} and a driver
 \tmtexttt{pwhg\_outfundriver.f} to interface between these and \mbox{POWHEG-BOX}
 common-blocks.
...
Given these inputs \mbox{POWHEG-BOX} will take care to evaluate the
...
The relevant routines for evaluate the combinatorics are  \tmtexttt{genflavreglist},
 ...
 \tmtexttt{rad\_nkinreg=1+(nlegborn-flst\_startcol+1)}.
  This variable is then exported via the \tmtexttt{pwg\_radiation} common-block
  included in the \tmtexttt{Radiation.h} header file.

  Notice: all variables with the \tmtexttt{rad\_} prefix are made available by
  including the \tmtexttt{Radiation.h} header file.
\\ 
 ...
 \item  The array \tmtexttt{flst\_alr(1...nlegreal,1...flst\_nalr)}  is
    ...
    defined in \cite{Frixione:2007vw}. These are computed in the following way:
    ...
    the index   \tmtexttt{k}   such that
     \tmtexttt{flst\_born(1...nlegborn,k)}   is equal to
      \tmtexttt{flst\_uborn(1...nlegborn,j)}. It may happen,
    ...
    position of the  \tmtexttt{nlegborn}   element of the list. At
  ...
  \item At this stage, an array of arrays of pointers
    ...
    graph multiplied by weight factors $S_i$, $S_{ij}$.
    These in turn have the structure described in section 2.4.1 of \cite{Frixione:2007vw}. In
...
\mbox{POWHEG-BOX} provides an independent facility to set the input parameters for
...
POWHEG-h in the current run will carry the prefix \tmtexttt{<prefix>-}
instead of
\tmtexttt{pwg}.\\
...
is reading an event file may get parameters of the POWHEG-h run, if it needs
too.
\\
We document here a typical input file \tmtexttt{powheginput.dat}:\\
\\
\tmtexttt{
...
\mbox{POWHEG-BOX}.  The numbering scheme is that of LHAGLUE interface,
...
\tmtexttt{ih1,ih2} can be 1 or a proton and -1 for an antiproton. In case the
user would choose the MLM pdf package, the entries \tmtexttt{lhans1,lhans2}
...
kinematics.
\\
The following parameters control the operation of the POWHEG-h program:\\
\\
...
\mbox{POWHEG-BOX}. Before the program starts generating events, the integral
...
In addiction to mandatory parameters presented above, POWHEG-h also accepts
other parameters. 
...
\tmtexttt{POWHEG-BOX} counters in \tmtexttt{pwgcounters.dat} file.
These are quite self-explanatory and we will not make here any further comment
about them.

\section{Random number generator}
POWHEG-BOX uses the \tmtexttt{RM48} random number generator,
...
affects the POWHEG-BOX random number sequence, both in the generation of events
...
for subsequent showering. POWHEG-BOX saves the hard event information
...
order to be used in conjunction with POWHEG-h.

The program can be run in three ways.
\begin{itemize}
\item
POWHEG-BOX generates hard events, and store them in a file.
An SMC reads the file and showers it.
\item
POWHEG-BOX is linked directly to the SMC.
In this case the events are generated and immediately showered,
without intermediate storage.
\item
POWHEG-BOX is run as a standalone program, and the produced hard
events are analyzed without showering. The output yields in this case
NLO distribution with NLL resummation of soft gluon effects.
\end{itemize}

\subsection{Storing the user events}\label{sec:lhef}
The easiest way to interface POWHEG-BOX to an SMC is to simply store the hard
events in a file (which we call the {\em event file}), 
and in a subsequent run read the events and process them with
the SMC. The format of the event file supported by POWHEG-BOX is the ``Standard
...
The user should take care of installing the HERWIG or PYTHIA program in the POWHEG-h directory.
...
\subsection{Interfacing POWHEG-h with a shower Monte Carlo program}
...
\ \ \ \ \ end}\\
\\
that are the only link to the POWHEG-h program. The main program should
...
The routine \tmtexttt {pwhginit} performs the initialization of POWHEG-h,
...
\subsection{POWHEG-h as a standalone program}
