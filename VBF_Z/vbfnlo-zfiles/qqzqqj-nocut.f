c      
      ...
      implicit none
c
c	Last modified for POWHEG by Barbara Jaeger:  Dec. 2011
C
...
c
      if (linit) then
         print*,'no qsqAmin in real emission'
...
c      if ((abs(p21(4,2)).lt.qsqAmin).or.(abs(p21(4,3)).lt.qsqAmin).or.
c     &    (abs(p43(4,2)).lt.qsqAmin).or.(abs(p43(4,3)).lt.qsqAmin)) then
c             
c         res(k,3) = 0d0
c         res(k,2) = 0d0      
c
...
c      if ((abs(p21(4,2)).lt.qsqAmin).or.(abs(p21(4,3)).lt.qsqAmin).or.
c     &    (abs(p43(4,2)).lt.qsqAmin).or.(abs(p43(4,3)).lt.qsqAmin)) then       
c         res(k,3) = res(k,3)*1d-20
c         res(k,2) = res(k,2)*1d-20
c      endif
