...
c     total cross section sanity check
     ...
     $     ,100d0)
      ...
      real *8 p_muminus(0:3),p_muplus(0:3)
...
!      write(*,*)'nhep',nhep
!      write(*,*)'idhep',idhep(1:6)
!      write(*,*)'isthep',isthep(1:6)
         ...
         do ihep=1,nhep
            if(isthep(ihep).eq.1) then
               ...
               elseif(idhep(ihep).eq.vdecaytemp2) then
                  p_muminus(0)=phep(4,ihep)
                  do mu=1,3
                     p_muminus(mu)=phep(mu,ihep)
                  enddo
               elseif(idhep(ihep).eq.-vdecaytemp2) then
                  p_muplus(0)=phep(4,ihep)
                  do mu=1,3
                     p_muplus(mu)=phep(mu,ihep)
                  enddo
               endif
            endif
         enddo
...
c     muminus transverse momentum
      pt_muminus=sqrt(p_muminus(1)**2 + p_muminus(2)**2)
      call get_pseudorap(p_muminus,eta_muminus)
c     lplus transverse momentum
      pt_muplus=sqrt(p_muplus(1)**2 + p_muplus(2)**2)
      call get_pseudorap(p_muplus,eta_muplus)
