...
c arrays to reconstruct jets
     ...
     #     ptrackin(0:3),ptrackout(0:3)
      ...
      real * 8 inv_mV
      real * 8  Wmass,Wwidth,Wmasslow,Wmasshigh,mV2ref,mV2
      logical foundlep
      integer nleps,nvls
      logical findmother
...
c     set values if analysis file is run by HERWIG and PYTHIA
            Wmass = 80.398d0
            Wwidth = 2.141d0
            Wmasslow = (Wmass-10*Wwidth)
            Wmasshigh = (Wmass+10*Wwidth)
         endif
...
c     MC@NLO
            if (isthep(ihep).eq.1.and.abs(idhep(ihep)).eq.vdecaytemp)
     1           then
               is_W = abs(idhep(jmohep(1,jmohep(1,ihep)))).eq.24
               if (.not.is_W) then
                  is_W = abs(idhep(jmohep(1,jmohep(1,jmohep(1
     $                 ,ihep))))).eq.24
               endif
               if (is_W) then
...
c correct here pvl and plep for collinear photons (in the electron case)

         if (nlep*nvl.eq.0) then
c     no two opposite-sign leptons to reconstruct the W mass peak
            return
         endif
...
c in case of electron, we may need to add collinear photons here
                  pvb(i)=plep(i,nleps)+pvl(i,nvls)
               enddo
               call getinvmass(pvb,inv_mV)
               if (inv_mV.gt.Wmasslow.and.inv_mV.lt.Wmasshigh.and.
     #                 abs(inv_mV-Wmass).lt.dist) then
                  dist = abs(inv_mV-Wmass)
                  ilep = nleps
                  ivl = nvls
               endif
            enddo
...
c     no reconstructed W mass peak in the experimental window
            return
         else            
            do i=1,4
               pvb(i)=plep(i,ilep)+pvl(i,ivl)
            enddo        
         endif         
      endif
