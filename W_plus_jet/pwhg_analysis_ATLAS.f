...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      diag=diag+1
      bsz(diag)=5d0
      call pwhgbookup(diag,'ATLAS antikt ptj1 >=1jet','LOG',
     $     bsz(diag),20d0,350d0)

      diag=diag+1
      bsz(diag)=5d0
      call pwhgbookup(diag,'ATLAS antikt ptj2 >=2jet','LOG',
     $     bsz(diag),20d0,190d0)

      diag=diag+1
      bsz(diag)=5d0
      call pwhgbookup(diag,'ATLAS antikt ptj3 >=3jet','LOG',
     $     bsz(diag),20d0,350d0)

      diag=diag+1
      bsz(diag)=5d0
      call pwhgbookup(diag,'ATLAS antikt ptj4 >=4jet','LOG',
     $     bsz(diag),20d0,190d0)

      diag=5
      bsz(diag)=1d0
      call pwhgbookup(diag,'ATLAS antikt total >=njet','LOG',
...
c     find lepton and neutrino from W (needed for the ATLAS analysis)
         ...
         do ihep=1,nhep
            if(isthep(ihep).eq.1) then
               if(abs(idhep(ihep)).eq.idlep) then
                  ilep=ihep
                  nlep=nlep+1
               elseif(abs(idhep(ihep)).eq.idnu) then
                  ivl=ihep
                  nvl=nvl+1
               endif
            endif
      ...
      elseif(WHCPRG.eq.'PYTHIA') then
         do ihep=1,nhep
            if(isthep(ihep).eq.1) then
               if((abs(idhep(ihep)).eq.idlep).and.
     $              (abs(idhep(jmohep(1,jmohep(1,ihep)))).eq.24)) then
                  ilep=ihep
                  nlep=nlep+1
               elseif((abs(idhep(ihep)).eq.idnu).and.
     $               (abs(idhep(jmohep(1,jmohep(1,ihep)))).eq.24)) then
                  ivl=ihep
                  nvl=nvl+1
               endif
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c     ATLAS analysis (arXiv:1012.5382)
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c     compute lepton and neutrino relevant variables
      do mu=1,4
...
c     'leptonic cuts'
      ...
      if(channel.eq.'electron') then
         ATLAS_eta_e = 2.47d0
         if((abs(eta_e).gt.1.37d0).and.
     $     (abs(eta_e).lt.1.52d0)) then
            goto 666            ! reject event (additional rejection for
      ...
      if((Et_e.lt.ATLAS_Et_e).or.
     $     (abs(eta_e).gt.ATLAS_eta_e).or.
     $     (Et_v.lt.ATLAS_Et_v).or.
     $     (Mt_w.lt.ATLAS_Mt_W)) then
         goto 666               ! reject event
      endif
