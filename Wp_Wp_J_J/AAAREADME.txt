...
When running on parallel CPU, make sure that each parallel run has a different
index
...
"fakevirt 1" in powheg.input 
Which means that the virtual calculation is replaced by a fake one proportional to the Born.  
...
can be performed with e.g. N (N\sim 50-100) runs with 2500-5000 calls (ncall2);
...
-- STEP 3) --
-------------
Can be run in parallel. The run must be performed in the same directory,
after all processes in STEP 2 have completed/
