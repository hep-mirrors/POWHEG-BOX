...
c     auxiliary variables
      ...
      if(u.le.1d-8) then
         ...
         if(v.ne.1d0) then
            myphi = (log(u)*log(v)+2d0*p_Li2(1d0-v))/(1d0-v)
         else
            myphi = 2d0-log(u)
         endif
      ...
      elseif(v.le.1d-8) then
         ...
         if(u.ne.1d0) then
            myphi = (log(v)*log(u)+2d0*p_Li2(1d0-u))/(1d0-u)
         else
            myphi = 2d0-log(v)
         endif
...
c     phi function from eq. (A4)
     ...
     &        2d0*(Dilog(cxp) + Dilog(cxm)) + Pi**2/3d0)/clam
         myphi = DBLE(ccphi)

c         write(*,*) 'cxp ', cxp, 'dilog(cxp) ', dilog(cxp)
c         write(*,*) 'cxm ', cxm, 'dilog(cxm) ', dilog(cxm)
c         write(*,*) 'ccphi', ccphi
c         stop
...
c$$$      implicit none
c$$$
c$$$      double complex SLCLI2,z
c$$$      double precision x,p_Li2
c$$$
c$$$      z = DCMPLX(x,0d0)
c$$$      p_Li2 = DBLE(SLCLI2(z))
c$$$
c$$$      return
c$$$      end
c$$$
...
c$$$*
c$$$
c$$$      DOUBLE COMPLEX FUNCTION SLCLI2(Z)
c$$$
