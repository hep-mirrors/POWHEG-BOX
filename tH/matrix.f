...
C  
C FOR PROCESS : g g -> b~ h- t  
C  
C Crossing   1 is g g -> b~ h- t  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C ----------
     ...
     &             NHEL(JJ,IHEL).EQ.INT(SIGN(1d0,POL(JJ)))) THEN
      ...
      REAL*8 FUNCTION MATRIXGG(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      CALL FSIXXX(W(1,3   ),W(1,4   ),GHMQ ,TMASS   ,TWIDTH  ,W(1,         
     &     6   ))                                                          
      CALL FVOXXX(W(1,5   ),W(1,1   ),GG ,TMASS   ,TWIDTH  ,W(1,7   ))     
      CALL IOVXXX(W(1,6   ),W(1,7   ),W(1,2   ),GG ,AMP(1   ))             
      CALL FVOXXX(W(1,5   ),W(1,2   ),GG ,TMASS   ,TWIDTH  ,W(1,8   ))     
      CALL IOVXXX(W(1,6   ),W(1,8   ),W(1,1   ),GG ,AMP(2   ))             
      CALL JVVXXX(W(1,1   ),W(1,2   ),G ,ZERO    ,ZERO    ,W(1,9   ))      
      CALL IOVXXX(W(1,6   ),W(1,5   ),W(1,9   ),GG ,AMP(3   ))             
      JAMP(   1) = +AMP(   1)+AMP(   3)
      JAMP(   2) = +AMP(   2)-AMP(   3)
      MATRIXGG = 0.D0 
      DO I = 1, NCOLOR
          ...
          ENDDO
          MATRIXGG =MATRIXGG+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
      Do I = 1, NGRAPHS
...
C FOR THE POINT IN PHASE SPACE P(0:3,NEXTERNAL)
C  
C FOR PROCESS : u u~ -> h- b~ t  
C  
C Crossing   1 is u u~ -> h- b~ t  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C ----------
     ...
     &             NHEL(JJ,IHEL).EQ.INT(SIGN(1d0,POL(JJ)))) THEN
      ...
      REAL*8 FUNCTION MATRIXQQ(P,NHEL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
C FOR THE POINT WITH EXTERNAL LINES W(0:6,NEXTERNAL)
...
C ----------
      ...
      CALL FSIXXX(W(1,4   ),W(1,3   ),GHMQ ,TMASS   ,TWIDTH  ,W(1,         
     &     7   ))                                                          
      CALL IOVXXX(W(1,7   ),W(1,5   ),W(1,6   ),GG ,AMP(1   ))             
      CALL JIOXXX(W(1,1   ),W(1,2   ),GAU ,ZERO    ,AWIDTH  ,W(1,8   ))    
      CALL IOVXXX(W(1,7   ),W(1,5   ),W(1,8   ),GAU ,AMP(2   ))            
      CALL JIOXXX(W(1,1   ),W(1,2   ),GZU ,ZMASS   ,ZWIDTH  ,W(1,9   ))    
      CALL IOVXXX(W(1,7   ),W(1,5   ),W(1,9   ),GZU ,AMP(3   ))            
      JAMP(   1) = +AMP(   1)
      JAMP(   2) = +AMP(   2)+AMP(   3)
      MATRIXQQ = 0.D0 
      DO I = 1, NCOLOR
          ...
          DO J = 1, NCOLOR
              ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
          ENDDO
          MATRIXQQ =MATRIXQQ+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
      Do I = 1, NGRAPHS
          amp2(i)=amp2(i)+amp(i)*dconjg(amp(i))
      Enddo
